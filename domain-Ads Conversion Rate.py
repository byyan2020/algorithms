...
class Solution:
    def ad_conversion(self, purchase_id, ad_clicks, user_ips):
        userIDset = set(purchase_id)
        ad_text = collections.defaultdict(list)
        for click in ad_clicks:
            parsed = click.split(',')
            ad_text[parsed[2]] += [parsed[0]]

        ip_user = {}
        for ip in user_ips:
            parsed = ip.split(',')
            ip_user[parsed[1]] = parsed[0]

        res = []
        for text, clicks in ad_text.items():
            buyer = 0
            for ip in clicks:
                if ip in ip_user and ip_user[ip] in userIDset:
                    buyer += 1
